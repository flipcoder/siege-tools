...
def obfuscate(project):
    ...
    os.system(
        "%s -jar %s %s"
        % (
            os.path.join(project.javapath, "java"),
            obf_path,
            os.path.join(os.getcwd(), "allatori.xml"),
        )
    )
...
def compatible(project):
    support = (
        Support.ENVIRONMENT | Support.USER | Support.AUTO
    )  # TODO: basic checks for user support before adding this flag
